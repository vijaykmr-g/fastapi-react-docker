...
function CrudPage({ token, onLogout }) {
...
const [newProduct,setNewProduct] = useState({
  ...
  const fetchproducts = async () => {
    const res = await axios.get("http://localhost:8000/products/",{headers: { Authorization: `Bearer ${token}` },});
  ...
  const handleaddsubmit = async (e) =>{
    e.preventDefault();
    try{
    await axios.post("http://localhost:8000/products",newProduct,{headers: { Authorization: `Bearer ${token}` },})
    alert("Product Added!");
    setAddPopup(false)
    setNewProduct({
